...
- (void)pickCharPressed:(UITapGestureRecognizer *)_sender
{
    ...
    NSString *path  = [[NSBundle mainBundle] pathForResource:@"plastic_iron_water_spray_button_press_empty_no_water" ofType:@"mp3"];
...
- (void)createTopInfo
{
    ...
    levelLabel.text = [NSString stringWithFormat:@"%d/100",playModel.level];
...
- (void)tapRightImgView:(UITapGestureRecognizer *)_sender
{
    ...
    [UIView animateWithDuration:0.2f
                     animations:^{
                         ...
                         UILabel *rightSourceLB = [[UILabel alloc]initWithFrame:CGRectMake(0, 270, 300, 30)];
                         [rightSourceLB setTextAlignment:NSTextAlignmentCenter];
                         rightSourceLB.text = playModel.rightSource;
                         [rightSourceLB setFont:[UIFont fontWithName:@"Arial-MT" size:25]];
                         rightSourceLB.adjustsFontSizeToFitWidth = YES;
                         //    [rightSourceLB setTransform:CGAffineTransformMakeRotation(-M_PI / 2)];
                         [rightSourceLB setBackgroundColor:[UIColor grayColor]];
                         rightSourceLB.alpha = 0.7;
                         [newView addSubview:rightSourceLB];
...
//    [clearView addSubview:tmpImgView];
    ...
    animations:^{
        ...
        UILabel *leftSourceLB = [[UILabel alloc]initWithFrame:CGRectMake(0, 270, 300, 30)];
        [leftSourceLB setTextAlignment:NSTextAlignmentCenter];
        leftSourceLB.text = playModel.leftSource;
//        [leftSourceLB setFont:[UIFont fontWithName:@"Arial-BoldMT" size:30]];
        leftSourceLB.adjustsFontSizeToFitWidth = YES;
        
        //    [rightSourceLB setTransform:CGAffineTransformMakeRotation(-M_PI / 2)];
        [leftSourceLB setBackgroundColor:[UIColor grayColor]];
        leftSourceLB.alpha = 0.7;
        [newView addSubview:leftSourceLB];
...
//    [blackView setHidden:YES];
    ...
    UILabel *leftSourceLB = [[UILabel alloc]initWithFrame:CGRectMake(5-kSizeOfImage/2, kYOfImage+kSizeOfImage/2-10, kSizeOfImage, 20)];
    [leftSourceLB setTextAlignment:NSTextAlignmentCenter];
    leftSourceLB.text = playModel.leftSource;
    ...
    UILabel *rightSourceLB = [[UILabel alloc]initWithFrame:CGRectMake(315-kSizeOfImage/2, kYOfImage+kSizeOfImage/2-10, kSizeOfImage, 20)];
    [rightSourceLB setTextAlignment:NSTextAlignmentCenter];
    rightSourceLB.text = playModel.rightSource;
...
//    asM = [[AnswerModel alloc]initWithNumCharOfLeft:3 NumCharOfRight:3 LeftString:@"cash" RightString:@"turtle" CorrectAnswer:@"castle"];
    ...
    NSLog(@"k yf of typing view : %d",kYOfTypingView);
        NSLog(@"k yf of typing view : %d",kSizeOfTypingSquare);
        NSLog(@"k yf of typing view : %d",kDistanceTypingSquare);
    NSLog(@"zzz %d",kYOfTypingView + kSizeOfTypingSquare + kDistanceTypingSquare-30);
    if (valueDevice <3){
...
- (void)correctAnswerFromSubView
{
    ...
    NSString *path  = [[NSBundle mainBundle] pathForResource:@"tada2" ofType:@"wav"];
    NSURL *pathURL = [NSURL fileURLWithPath : path];
    AudioServicesCreateSystemSoundID((CFURLRef) pathURL, &audioEffect);
    AudioServicesPlaySystemSound(audioEffect);
